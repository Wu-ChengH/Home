...
(function (window, undefined) {
	...
	S.downLoad=function(api,params){
		var form=$("<form target='' method='get' action='"+api+"' name='downLoad'></form>");
	...
	S.log = function (msg, type) {
        if (!document.all&&console&&S.debug==true) {
            switch (type) {
                case "error":
...
　　};
	...
	S.asyncLoader = function (file, type, total, callBack) {
        ...
        if (type == "js") {
            ...
            S.cssLoader(file[S.temp[type + "CurrentIndex"]], function () {
                S.temp[type + "CurrentIndex"] = S.temp[type + "CurrentIndex"] + 1;
                S.asyncLoader(file, type, total, callBack);
            });
        }
    ...
    S.request = function (option) {
        ...
        /*if(option.url == seft_serverUrl +"/api/login"||option.url == seft_serverUrl +"/api/resetting/validateCode"||option.url == seft_serverUrl +"/api/resetting/password"){
            var bearer = ""
        }else{
        ...
        $.ajax({
            ...
            data:JSON.stringify(option.data) || JSON.stringify(),
            url:option.url|| '',
            async:async,
            dataType:option.dataType || 'json',
            contentType:"application/json",
            /*headers:{
                Authorization: bearer, // 从sessionStorage中获取access token
            },*/
			timeout : option.timeout||20000, //超时时间设置，单位毫秒,默认设置10秒超时
            ...
            error:function (ex) {
                ...
                if(ex.status != 0){
                    var index = ex.responseJSON.message.lastIndexOf(":");
                    var str = ex.responseJSON.message.substring(index+1);
                    S.alert(str);                    
                }else{
                    S.alert("网络连接失败")
                }
	...
	S.alert = function(msg,callback){
		if(zDialog){
			if ($(".zDialog-title").length>0&&$(".zDialog-title").text() == "系统消息") {
        isloaded = false;
        $(".zDialog-body").each(function(i,v){
          if ($(v).text() == msg) {
            isloaded = true;
          };
        })
