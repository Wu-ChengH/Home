...
(function(window,undefined){
	...
	zDialog.prototype = {
		...
		setStyle : function(wraper){
			...
			marginLeft = dialogW/2+5;
			marginTop = dialogH/2+5;
					...
					case "page" :
						...
						wraper.find(".zDialog-body").load(d.get("url"),function(tplResult,status){
							if(status!="success"){
								...
								d.setStyle(wraper);
								var pageCallBack = {};
								pageCallBack = new zDialog.pageCallBack(wraper,d.get("data"));
								if(typeof d.get("onComplete")=="function") d.get("onComplete")(wraper.find(".zDialog-body"));
							}
						})
	...
	$.zAlert = function(msg,callBack){
		if ($(".zDialog-title").length>0&&$(".zDialog-title").text().indexOf("系统消息")>=0) {
			isloaded = false;
	...
	$.zUpdate = function(msg,sure,notSure){
		if ($(".zDialog-title").length>0&&$(".zDialog-title").text().indexOf("更新提示")>=0) {
			return;
		}else{
			return new zDialog({
				title:"更新提示",
